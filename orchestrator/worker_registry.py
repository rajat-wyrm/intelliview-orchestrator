...
- Maintain real-time multi-instance cache sync via Redis Pub/Sub
"""
import asyncio
import json
import logging
    ...
    def __init__(self):
        ...
        try:
            ...
            if self.redis_client:
                task = asyncio.create_task(self._start_pubsub_listener())
    ...
    def _get_native_redis_client(self) -> Any:
        ...
        for _ in range(3):
            ...
            if hasattr(client, "_client"):
                client = getattr(client, "_client")
            elif hasattr(client, "_redis"):
                client = getattr(client, "_redis")
            elif hasattr(client, "redis"):
                client = getattr(client, "redis")
    ...
    def heartbeat(self, worker_id: str, active_tasks: int) -> bool:
        ...
        try:
            ...
            with self.lock:
                ...
                if old_worker:
                    if (old_worker.get("active_tasks") != active_tasks or
                            old_worker.get("status") != "healthy"):
                        has_changed = True
                else:
                    has_changed = True
