...
def test_pubsub_sync_message_updates_local_workers():
    ...
    fake_message = {
        "type": "message",
        "data": '{"worker_id": "test_sync_worker", "action": "sync"}'
    }
    with patch.object(reg.redis_client, 'hgetall', return_value={
        "status": "healthy", "active_tasks": "2", "capacity": "4"
    }):
...
def test_pubsub_deregister_message_removes_worker():
    ...
    fake_message = {
        "type": "message",
        "data": '{"worker_id": "dead_worker", "action": "deregister"}'
    }
    reg._handle_pubsub_message(fake_message)
